...
class HttpClientNlp extends HttpClient {
        ...
        return new Promise(function(resolve, reject) {
            request(options, function(error, response, body) {
                if (error === null) {
                    ...
                    let decoder = new TextDecoder(options.encodeType || code.GBK);
                    let decodedBody = decoder.decode(buffer);
                    //let decodedBody = iconv.decode(buffer, options.encodeType || code.GBK);
                    try {
    ...
    postWithInfo(requestInfo) {
        let body = this.createBody(requestInfo);
        let options = {
            encodeType: requestInfo.encodeType,
    ...
    createBody(requestInfo) {
        let body = null;
        let encoder = new TextEncoder(requestInfo.encodeType || code.GBK);
        body = encoder.encode(JSON.stringify(requestInfo.params));
        //body = iconv.encode(JSON.stringify(requestInfo.params), requestInfo.encodeType || code.GBK);
        return body;
    }
}
